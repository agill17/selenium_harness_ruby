Given(/^I click the "(.*?)" (radio|checkbox|button) "(.*?)"$/) do | ele,type,attr_type |
  ...
  when "class"
    element = Find_Element.by_class(Locators::Home_page["#{attr_type}"]["type"]["ele"], @driver)
  when "text"
    element = Find_Element.by_text(Locators::Home_page["#{attr_type}"]["type"]["ele"], @driver)
  when "name" 
    element = Find_Element.by_name(Locators::Home_page["#{attr_type}"]["type"]["ele"], @driver)
  end
  element.click
end
